      ...
      <div class="drawer-inner-container view-drawer-container">
        <el-card shadow="hover" class="simple-view-card">
          <el-descriptions :column="1">
            ...
            <el-descriptions-item label="提交时间">{{ formatTime(currentApproval.createdAt) }}</el-descriptions-item>
            <el-descriptions-item label="审批时间">{{ formatTime(currentApproval.updatedAt) }}</el-descriptions-item>
          </el-descriptions>
        </el-card>
      </div>
...
const handleStatusChange = (status) => {
  ...
  // 确保始终查询权限审批
  searchForm.approval_type = 'permission'
...
const submitApprovalForm = async () => {
  ...
  try {
    ...
    // 直接传递status值，不需要包装成对象
    await approvalStore.UpdateApprovalStatus(currentApproval.value.id, approvalForm.status);
